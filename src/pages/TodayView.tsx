...
import { Zap, Sun, Moon, Battery, RefreshCw, ClipboardList } from 'lucide-react';
...
import { useScheduleBlocks, useCreateBlocks, useUpdateBlock, useDeleteBlocks, useDeleteBlocksByIds } from '../hooks/useScheduleBlocks';
...
import { buildAndGenerateSchedule, buildAndReshuffleSchedule } from '../lib/scheduleAdapter';
...
export default function TodayView() {
  ...
  // Auto-generate schedule on first load with medium energy (no gate)
  useEffect(() => {
    if (!checkin && !blocksLoading && !blocks?.length && !autoGenerated && profile && categories) {
      setAutoGenerated(true);
      handleEnergySelect('medium');
    }
  }, [checkin, blocksLoading, blocks, autoGenerated, profile, categories, handleEnergySelect]);

  // Mid-day energy change: reshuffle only remaining blocks
  const handleEnergyChange = useCallback(async (newLevel: EnergyLevel) => {
    if (!profile || !categories || !blocks) return;

    // Update checkin
    await upsertCheckin.mutateAsync({
      date: today, energy_level: newLevel,
      wake_time: profile.default_wake_time, wind_down_time: profile.default_wind_down_time,
    });

    // Determine which blocks to keep vs remove
    ...
    const removeBlockIds = blocks
      .filter((b) => !keepBlocks.includes(b))
      .map((b) => b.id);

    // Generate new blocks for remaining time
    const newBlocks = buildAndReshuffleSchedule({
      profileId, profile, categories, energyLevel: newLevel, date: today,
      tasks: tasks || [], recurringTasks: recurringTasks || [],
      travelTimes: travelTimes || [], locations: locations || [],
      neglectScores, existingBlocks: blocks,
    });

    // Delete removed blocks, insert new ones
    if (removeBlockIds.length > 0) {
      await deleteBlocksByIds.mutateAsync(removeBlockIds);
    }
    if (newBlocks.length > 0) {
      await createBlocks.mutateAsync(newBlocks);
    }

    setShowEnergyModal(false);
    toast.success(`Energy updated to ${newLevel} — schedule adjusted`);
  }, [profileId, profile, categories, blocks, today, tasks, recurringTasks, travelTimes, locations, neglectScores, upsertCheckin, deleteBlocksByIds, createBlocks]);
